...
if [ "$1" == "kill" ]; then
  docker-compose -f docker-compose-mock.yml kill
  exit 0
fi
...
if [ "$1" == "run" ]; then
  ...
  nohup docker-compose -f docker-compose-mock.yml up 2>&1 &

  # Wait for system to be fully up
  start_epoch="$(date -u +%s)"
  elapsed_seconds=0
  until [ $(docker-compose -f docker-compose-mock.yml ps | grep Up | wc -l) -eq 4 ] || [ $elapsed_seconds -ge 600 ]
  do
    sleep 30
    docker-compose -f docker-compose-mock.yml ps
